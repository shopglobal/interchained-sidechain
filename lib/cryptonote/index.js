...
class Wallet {
  constructor(hostname, port, user, pass) {
    ...
    this.user = user || '';
    this.pass = pass || '';
    this._request('get_balance'); // This line is necessary in order to do the initial handshake between this wrapper and electronero-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
    }
};
/**
 * Wallet Request 
...
Wallet.prototype._request = function (method, params = '') {
  ...
  return request.post(`http://${this.hostname}:${this.port}/json_rpc`, options).then((result) => {
...
Wallet.prototype.create_wallet = function(filename, password, language) {
    ...
    let params = {
        filename: filename || 'electronero_wallet',
        'password': password || '',
...
// transfer Electronero to a single recipient, or a group of recipients
...
// helper function to convert Electronero coins amount to atomic units
// default coin === 'crfi' || 1e12 === 12 decimals 
Wallet.prototype.convert = function(amount,coinSymbol='crfi') {
  const coinUnits = coinSymbol==="etnxp" ? 1e6 : coinSymbol==="etnx" ? 1e8 : coinSymbol==="ltnx" ? 1e8 : coinSymbol==="gldx" ? 1e12 : coinSymbol==="crfi" ? 1e12 : 1e12;
    let number = Number(amount) * 1e12;
    // remove decimals, to process transactions 
    number = number.toFixed(0);
  return Number(number);
}
// helper function to convert Electronero coins amount atomic units to a human read-able format
// default to 'crfi' || 1000000000000 if !coinSymbol
Wallet.prototype.formatCoins = function(coins,coinSymbol='crfi'){
  const coinUnits = coinSymbol==="etnxp" ? 1000000 : coinSymbol==="etnx" ? 100000000 : coinSymbol==="ltnx" ? 100000000 : coinSymbol==="gldx" ? 1000000000000 : coinSymbol==="crfi" ? 1000000000000 : 1000000000000; 
    var balance = coins * coinUnits;
  return balance;
};
...
// HELPERS
// helper function to convert Electronero amount to atomic units
// default coin === 'crfi' || 1e12 === 12 decimals 
function convert(amount,coinSymbol='crfi') {
  const coinUnits = coinSymbol==="etnxp" ? 1e6 : coinSymbol==="etnx" ? 1e8 : coinSymbol==="ltnx" ? 1e8 : coinSymbol==="gldx" ? 1e12 : coinSymbol==="crfi" ? 1e12 : 1e12;
    let number = Number(amount) * coinUnits;
    // remove decimals, to process transactions 
    number = number.toFixed(0);
  return Number(number);
}
// helper function to convert Electronero coins amount atomic units to a human read-able format
// default to 'crfi' || 1000000000000 if !coinSymbol
function formatCoins(coins,coinSymbol='crfi'){
  const coinUnits = coinSymbol==="etnxp" ? 1000000 : coinSymbol==="etnx" ? 100000000 : coinSymbol==="ltnx" ? 100000000 : coinSymbol==="gldx" ? 1000000000000 : coinSymbol==="crfi" ? 1000000000000 : 1000000000000; 
    var balance = coins * coinUnits;
  return balance;
};
